...
uniform int u_mode; // 0: Complex Domain, 1: Fractals, 2: Flow Field, 3: Linear Algebra
...
// --- Domain 0: Complex Domain Coloring f(z) = z^3 - 1 + sin(t) ---
vec3 renderComplexDomain(vec2 z) {
  ...
  return hsl2rgb(vec3(hue, 0.85, clamp(val * 0.6, 0.15, 0.85)));
}

// --- Domain 1: Mandelbrot / Julia Fractals ---
vec3 renderFractal(vec2 c) {
  ...
  if (n >= maxIter) return vec3(0.0);
  
  // Smooth coloring
  float nu = log(log(dot(z, z)) / 2.0) / log(2.0);
  float iter = n + 1.0 - nu;
  
  return hsl2rgb(vec3(iter * 0.05 + u_time * 0.05, 0.8, 0.5));
}

// --- Domain 2: Vector Flow Field ---
vec3 renderFlowField(vec2 p) {
  // Vector field: V(x,y) = (sin(y + t), cos(x + t))
...
// --- Domain 3: Linear Algebra Grid Transformations ---
vec3 renderLinearTransform(vec2 p) {
  // Matrix transformation: A = [[cos(t), -sin(t)], [sin(t), cos(t)]]
  float a = u_time * 0.5;
  ...
  if (abs(transformed.x) < 0.05) axisColor = vec3(0.2, 0.8, 1.0); // Y-axis
  if (abs(transformed.y) < 0.05) axisColor = vec3(1.0, 0.3, 0.4); // X-axis
